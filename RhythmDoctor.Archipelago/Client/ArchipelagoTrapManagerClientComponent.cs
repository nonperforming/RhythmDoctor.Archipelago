...
{
  private List<string> _trapQueue;
  private List<(int index, string Uid)> _trapAndIndexPairs;
  ...
  internal void ReturnActiveTrapsToQueue()
  {
    ...
    for (int i = _trapAndIndexPairs.Count - 1; i >= 0; i--)
    {
      (int index, string uid) = _trapAndIndexPairs[i];

      Plugin.Logger.LogDebug(
        $"[{nameof(ArchipelagoTrapManagerClientComponent)}] Returning trap {uid} to index {index}."
      );
      _trapQueue.Insert(index, uid);
    }
    _trapAndIndexPairs.Clear();
  }
  ...
  protected override float GetModifierStrength(IModifier modifier)
  {
    ...
    List<int> matchIndexes = _trapQueue
    ...
    for (int i = 0; i < consumed; i++)
    {
      ...
      _trapAndIndexPairs.Add((indexToRemove, modifier.Uid));
    }
