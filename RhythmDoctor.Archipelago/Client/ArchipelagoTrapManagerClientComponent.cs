...
internal sealed class ArchipelagoTrapManagerClientComponent : ModifierManagerBase, IClientComponent, IDisposable
{
  ...
  internal async Task AddTrap(ITrap trap, bool doNotCheckPriorClear = false)
  {
    ...
    async Task<bool> CheckIfTrapAlreadyCleared(string trapName)
    {
      ...
      Plugin.Logger.LogInfo(
        $"[{nameof(ArchipelagoTrapManagerClientComponent)}] Checking if {trapName} has been cleared previously..."
      );
      ...
      {
        Plugin.Logger.LogInfo(
          $"[{nameof(ArchipelagoTrapManagerClientComponent)}] Waiting for semaphore to check DataStorage for {trapName}..."
        );
        await _gettingTrapClearedSemaphore.WaitAsync();
        try
        {
          Plugin.Logger.LogInfo(
            $"[{nameof(ArchipelagoTrapManagerClientComponent)}] Got semaphore, checking DataStorage for {trapName}..."
          );
        ...
        {
          Plugin.Logger.LogDebug(
            $"[{nameof(ArchipelagoTrapManagerClientComponent)}] Releasing semaphore (used for {trapName})"
          );
      ...
      Plugin.Logger.LogDebug(
        $"[{nameof(ArchipelagoTrapManagerClientComponent)}] {trapName} not cleared: l:{local} <= r:{remoteCache} != 0"
      );
      return false;
    }
    ...
    {
      Plugin.Logger.LogInfo(
        $"[{nameof(ArchipelagoTrapManagerClientComponent)}] Skipping {trap.Name} trap as it has been cleared previously"
      );
  ...
  {
    Plugin.Logger.LogInfo(
      $"[{nameof(ArchipelagoTrapManagerClientComponent)}] Clearing traps list (return to queue: {returnToQueue})"
    );

    if (returnToQueue)
    {
      Plugin.Logger.LogInfo(
        $"[{nameof(ArchipelagoTrapManagerClientComponent)}] Restoring {trapList.Length} traps back into the trap queue"
      );
  ...
  {
    Plugin.Logger.LogInfo(
      $"[{nameof(ArchipelagoTrapManagerClientComponent)}] Clearing active traps (return to queue: {returnToQueue})"
    );

    if (_activeTraps.Length == 0)
    {
      Plugin.Logger.LogWarning(
        $"[{nameof(ArchipelagoTrapManagerClientComponent)}] There must be at least one active trap to clear."
      );
      return;
    }
  ...
  internal void PromotePreviewTrapsToActiveTraps()
  {
    ...
    {
      Plugin.Logger.LogError(
        $"[{nameof(ArchipelagoTrapManagerClientComponent)}] There must be no active traps currently applied. "
          + "Discarding old active traps."
      );
